...
find "$CURRENTDIR" -maxdepth 1 -mindepth 1 -type f \( -iname "*.md" !  -iname "_coverpage.md" !  -iname ".*" ! -iname "README.md" ! -iname "_navbar.md"  ! -iname "index.html"  ! -iname "index.html"  ! -iname "_sidebar.md" \)| sort | while read filepath; do
 wholepath=$filepath
...
find "$CURRENTDIR" -maxdepth 1 -mindepth 1 -type d \( ! -iname ".*" ! -iname "static" ! -iname "includes" \)| sort | while read filepath; do 
...
#   打印文件夹
  ...
  echo "$tabs*  **$folderName**"

  echo "$tabs*  [**📖**]($folderPath/)"
